...
- Fixed the score-to-odds table treating raw 30 as a losing score: the forecast now normalizes it to zero, and the UI lists “Shoot the moon → 0” beside the zero row with identical win/push/EV values. Score thresholds exclude the duplicate moon row. Verified exact equivalence and a guaranteed push against a posted zero.
