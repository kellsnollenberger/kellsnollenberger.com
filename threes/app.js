import {points} from './engine.js?v=13';
import {playoffScoreValues,liveRoundChances} from './playoffs.js?v=13';
...
function render(){
  ...
  if(phase==='ready'){
    $('phase').textContent='TAKE YOUR SEAT';$('headline').textContent='Let the low rolls win.';$('message').textContent='Threes count as zero. Five sixes also score zero.';$('action').textContent=`Deal me in · ${money(ante)} ante`;$('hint').textContent='Play money only · strategy shown automatically';
  }else if(phase==='bust'){
    ...
    $('action').textContent=selection.size===dice.length?'Lock dice & post score':selection.size?'Hold selected & roll':'Select dice to hold';$('hint').textContent=locked.length&&locked.every(f=>f===6)?`Moon chase: ${locked.length} six${locked.length===1?'':'es'} saved. Complete five sixes to score zero.`:'Tap dice to select · at least one must stay · threes are zero · five sixes shoot the moon';
  }else{
