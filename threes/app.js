import {points} from './engine.js?v=5';
import {playoffScoreValues} from './playoffs.js?v=5';
...
function renderLens(myTurn){
  ...
  const explanation=bestWin.ev<best.ev-0.005?`The highest win-now row keeps ${holdText(bestWin)} (${pct(bestWin.win)} now), but keeping ${holdText(best)} has a better expected return by ${money(best.ev-bestWin.ev)} after playoff value and extra antes.`:bestAlive.ev<best.ev-0.005?`Keeping ${holdText(bestAlive)} stays alive more often (${pct(bestAlive.survive)}), but keeping ${holdText(best)} returns ${money(best.ev-bestAlive.ev)} more on average. A playoff entry is worth less than a win now.`:'This hold gives the highest modeled return after valuing playoff wins and extra antes.';
  const short=game.order.some(i=>game.players[i].bank<game.ante);
  $('lensBody').innerHTML=`<div class="analysis"><p><b>Play for expected return</b><br>${advisor.known.length?`Posted target: ${advisor.target}. `:''}${advisor.future} opponent${advisor.future===1?'':'s'} still to act.</p><div class="recommend"><span class="eyebrow">BEST EV${options.filter(h=>joint(h,best)).length>1?' · JOINT BEST':''}</span><strong>Keep ${holdText(best)}</strong><strong>${signedMoney(best.ev)} expected round profit</strong><small>${pct(best.win)} win now + ${pct(best.viaPlayoff)} win through a playoff = ${pct(best.eventual)} total win</small><br><button id="useBest" class="quiet" style="margin-top:12px">Select this hold</button></div><p>${explanation}</p><p><b>Highest win-now row:</b> keep ${holdText(bestWin)} · ${pct(bestWin.win)}<br><b>Highest win-or-tie row:</b> keep ${holdText(bestAlive)} · ${pct(bestAlive.survive)}</p><div class="ev-breakdown"><p><b>Playoff entry:</b> ${pct(best.tie)}<br><b>Win it if you enter:</b> ${best.tie>1e-12?pct(best.viaPlayoff/best.tie):'— (no playoff entry)'}<br><b>Expected extra antes:</b> ${money(best.extraCost)}<br><b>Already paid this round:</b> ${money(game.paid[0])}</p></div><div class="option-list"><table><thead><tr><th>HOLD</th><th>WIN NOW</th><th>PLAYOFF</th><th>TOTAL WIN</th><th>NET EV</th></tr></thead><tbody>${options.map(h=>`<tr class="${joint(h,best)?'best':joint(h,worst)?'worst':''}"><td>${holdText(h)}${h.faces.join(',')===selected?' ✓':''}<span class="badge">${joint(h,best)?'BEST EV':joint(h,worst)?'WORST EV':''}</span><br><small>${h.mean.toFixed(2)} avg</small></td><td>${pct(h.win)}</td><td>${pct(h.tie)}</td><td>${pct(h.eventual)}</td><td>${signedMoney(h.ev)}</td></tr>`).join('')}</tbody></table></div><p>NET EV = expected full-pot payout minus all your antes, including future playoff antes. Each row assumes EV-advised play on your remaining rolls this turn. AVG projects a full five-dice score even on branches that bust. Total win includes wins now and through any number of playoffs; a playoff entry is not counted as a win.</p><details class="model-note"><summary>How the playoff estimate works</summary><p>The pot carries forward; only tied players re-ante and their order reverses. Multiway and repeated ties are included. Opponents follow the actual computers’ target-aware policy. Future playoff turns assume that same policy for everyone, including you. Your remaining rolls in this turn are optimized for EV; following improved advice in later playoffs can change the results. This is the best current-turn return under that model, not a universal optimum.</p><p>Assumes everyone can cover required playoff antes. Your current ante is already paid and counts toward round profit; it does not change which hold is best.</p></details>${short?'<p class="funding-note">At least one seat cannot cover another ante. A tie involving that seat will pause the actual game; modeled playoff returns assume it can be funded.</p>':''}</div>`;
...
function renderScoreOdds(){
  ...
  $('scoreOddsBody').innerHTML=`<p><b>${context}</b></p>${Number.isFinite(target)?`<p>Beat <b>${target}</b> to win now; match it for a possible playoff.</p>`:''}<div class="score-examples">${[2,4].map(score=>`<div><small>FINISH WITH ${score}</small><strong>${pct(rows[score].eventual)}</strong><span>total win, including playoffs<br>${pct(rows[score].win)} win now<br>${signedMoney(rows[score].ev)} net EV</span></div>`).join('')}</div><p>${goals.join('<br>')}</p><div class="option-list"><table><thead><tr><th scope="col">SCORE</th><th scope="col">WIN NOW</th><th scope="col">PLAYOFF</th><th scope="col">TOTAL WIN</th><th scope="col">NET EV</th></tr></thead><tbody>${rows.map(r=>`<tr class="${r.mean===2||r.mean===4?'example-row':''}"><th scope="row">${r.mean}</th><td>${pct(r.win)}</td><td>${pct(r.tie)}</td><td>${pct(r.eventual)}</td><td>${signedMoney(r.ev)}</td></tr>`).join('')}</tbody></table></div><p>Conditional on already scoring that total, not the chance of rolling it. NET EV includes the current ante and expected extra playoff antes. Forecasts target-aware computer play, reversed playoff order, and repeated ties. Future playoff turns assume the computer policy for everyone and sufficient bankroll to re-ante; these are model-based estimates. Live rows replace your score hypothetically.</p>`;
}
